...
- **Keyword-backed rule resolution** for pseudo-rules like `UnreservedKeyword` via `grammar/keywords/*.list` when no `.gram` definition exists
...
- Generated `inlined_grammar.gram` files resolve their own rules when open, and keyword-category references can fall back to `grammar/keywords/*.list`.
...
- `inlined_grammar.gram` remains excluded from global workspace indexing to avoid duplicate generated definitions, but the open file still resolves its own rules.
